...
*/
void Jump() {
//	sndPlaySound(TEXT("C:\\sample.wav"), SND_ASYNC | SND_NOSTOP);//배경믕악
...
void Show()
{
	...
	glLoadIdentity(); //CTM 초기화
					  ...
					  로봇의 기본적인 관절의 움직임 범위를 제한하는 곳
					  */
	L_Arm_x = (-40) + sin(time) * 60;//왼쪽 어깨의 각도시작은 -40상태에서 sin()함수를 사용하여 주기적인 움직임 설정
	R_Arm_x = (-80) - L_Arm_x;       //우측 어깨의 각도시작은 -80상태에서 왼쪽어깨 움직임의 반대로 설정
	R_Arm_y = -abs(long(cos(time)) * 10);  //우측팔뚝 각도조절(팔을 뻗는 움직임표현을위하여 어깨의 sin()함수와 반대인 cos()함수 사용)
	L_Arm_y = -abs(long(-cos(time)) * 10); //좌측팔뚝 각도조절(팔을 뻗는 움직임표현을위하여 어깨의 sin()함수와 반대인 cos()함수 사용)
...
*/
void MyDisplay() {
	if (key == JUMP) {//로봇이 점프할 때 
		Jump();
		glPopMatrix();
	}
	else if (key == RUN) {//로봇이 달릴때		
		Run();
		glPopMatrix();
	}
...
//	InitLight2();
	...
	glutAddMenuEntry("JUMP", 7);//로봇이 점프하는 모션 메뉴 추가 
